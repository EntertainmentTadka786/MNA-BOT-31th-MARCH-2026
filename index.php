...
define('CSV_FILE', getenv('CSV_FILE') ? getenv('CSV_FILE') : 'movies.csv');
define('USERS_FILE', getenv('USERS_FILE') ? getenv('USERS_FILE') : 'users.json');
define('STATS_FILE', getenv('STATS_FILE') ? getenv('STATS_FILE') : 'bot_stats.json');
define('WAITING_FILE', getenv('WAITING_FILE') ? getenv('WAITING_FILE') : 'waiting.json');
define('BACKUP_DIR', getenv('BACKUP_DIR') ? getenv('BACKUP_DIR') : 'backups/');
...
if ($publicChannelsStr) {
    ...
    foreach ($pairs as $pair) {
        $parts = explode('|', $pair);
        $publicChannels[trim($parts[1])] = trim($parts[0]);
    }
}
...
$privateChannels = $privateChannelsStr ? array_map('trim', explode(',', $privateChannelsStr)) : array();
...
if (!file_exists(USERS_FILE)) {
    file_put_contents(USERS_FILE, json_encode(array('users' => array(), 'total_requests' => 0)));
...
if (!file_exists(WAITING_FILE)) {
    file_put_contents(WAITING_FILE, json_encode(array()));
...
function update_stats($field, $increment = 1) {
    ...
    $stats[$field] = (isset($stats[$field]) ? $stats[$field] : 0) + $increment;
...
function set_last_run($type, $timestamp = null) {
    $stats = get_stats();
    $stats["last_$type"] = $timestamp ? $timestamp : date('Y-m-d');
    file_put_contents(STATS_FILE, json_encode($stats, JSON_PRETTY_PRINT));
}

function should_run_today($type) {
    $stats = get_stats();
    $last = isset($stats["last_$type"]) ? $stats["last_$type"] : '';
...
function get_cached_movies() {
    ...
    $movie_cache = array('data' => load_and_clean_csv(), 'timestamp' => time());
    return $movie_cache['data'];
}
...
function smart_search($query) {
    ...
    foreach ($movie_messages as $movie => $entries) {
        ...
        if ($score > 0) {
            $results[$movie] = array('score' => $score, 'count' => count($entries));
        }
    }
    uasort($results, create_function('$a,$b', 'return $b["score"] - $a["score"];'));
...
function detect_language($text) {
    $hindi_keywords = array('फिल्म', 'मूवी', 'डाउनलोड', 'हिंदी');
    $english_keywords = array('movie', 'download', 'watch', 'print');
    $hindi_count = 0;
    $english_count = 0;
    foreach ($hindi_keywords as $kw) {
        if (mb_strpos($text, $kw) !== false) $hindi_count++;
    }
    foreach ($english_keywords as $kw) {
        if (stripos($text, $kw) !== false) $english_count++;
    }
...
function auto_backup() {
    ...
    foreach (array(CSV_FILE, USERS_FILE, STATS_FILE, WAITING_FILE) as $file) {
        if (file_exists($file)) {
            copy($file, $backup_dir . '/' . basename($file) . '.bak');
        }
    }
    $old_backups = glob(BACKUP_DIR . '*', GLOB_ONLYDIR);
    usort($old_backups, create_function('$a,$b', 'return filemtime($a) - filemtime($b);'));
    while (count($old_backups) > 7) {
        delete_directory(array_shift($old_backups));
    }
    set_last_run('backup');
}
...
function load_and_clean_csv() {
    ...
    if ($handle) {
        ...
        while (($row = fgetcsv($handle)) !== false) {
            if (count($row) >= 4 && is_numeric($row[1]) && !empty(trim($row[0]))) {
                ...
                if (!isset($movie_messages[$movie])) $movie_messages[$movie] = array();
                $movie_messages[$movie][] = array('msg_id' => $entry['message_id'], 'channel_id' => $entry['channel_id']);
            }
        }
        fclose($handle);
    }
    ...
    fputcsv($handle, array('movie_name', 'message_id', 'channel_id', 'date'));
    foreach ($data as $row) {
        fputcsv($handle, array($row['movie_name'], $row['message_id'], $row['channel_id'], $row['date']));
    }
    fclose($handle);
    return $data;
}
...
function append_movie($movie_name, $message_id, $channel_id) {
    ...
    fputcsv($handle, array(trim($movie_name), intval($message_id), $channel_id, $date));
    fclose($handle);
    global $movie_messages;
    $movie = strtolower(trim($movie_name));
    if (!isset($movie_messages[$movie])) $movie_messages[$movie] = array();
    $movie_messages[$movie][] = array('msg_id' => intval($message_id), 'channel_id' => $channel_id);
...
function load_movies_from_csv() {
    ...
    if ($handle) {
        ...
        while (($row = fgetcsv($handle)) !== false) {
            if (count($row) >= 4) {
                $movies[] = array('name'=>$row[0], 'message_id'=>$row[1], 'channel_id'=>$row[2], 'date'=>$row[3]);
            }
        }
        fclose($handle);
    }
...
function advanced_search($chat_id, $user_id, $query) {
    ...
    if (strlen($query_lower) < 2) {
        sendMessage($chat_id, "❌ At least 2 characters");
        return;
    }
    ...
    if (!empty($found)) {
        ...
        $i = 1;
        foreach ($found as $movie => $info) {
            $msg .= "$i. $movie ({$info['count']})\n";
            $i++;
            if ($i > 15) break;
        }
        ...
        foreach ($top as $movie) {
            $keyboard['inline_keyboard'][] = array(array('text' => "🎬 " . ucwords($movie), 'callback_data' => $movie));
        }
...
function admin_stats($chat_id) {
    ...
    $msg = "📊 <b>Bot Statistics</b>\n\n🎬 Movies: {$stats['total_movies']}\n👥 Users: $total_users\n🔍 Searches: {$stats['total_searches']}\n🕒 Last Updated: {$stats['last_updated']}\n💾 Last Backup: " . (isset($stats['last_backup']) ? $stats['last_backup'] : 'Never') . "\n📅 Last Digest: " . (isset($stats['last_digest']) ? $stats['last_digest'] : 'Never') . "\n\n📈 Recent Uploads:\n";
    $recent = array_slice(load_and_clean_csv(), -5);
    foreach ($recent as $m) {
        $msg .= "• {$m['movie_name']} ({$m['date']})\n";
    }
...
function apiRequest($method, $params=array()) {
    ...
    error_log("API Error $method: " . (isset($decoded['description']) ? $decoded['description'] : 'Unknown'));
    return false;
}
...
function forwardMessage($chat_id, $from_chat_id, $message_id) {
    $res = apiRequest('forwardMessage', array(
        'chat_id' => $chat_id,
...
function answerCallbackQuery($id, $text=null) {
    $data = array('callback_query_id' => $id);
...
function total_uploads($chat_id, $page=1) {
    ...
    if ($total_pages > 1) {
        $keyboard = array('inline_keyboard' => array());
        $row = array();
        if ($current_page > 1) $row[] = array('text' => '⏮️ Previous', 'callback_data' => 'uploads_page_' . ($current_page - 1));
        if ($current_page < $total_pages) $row[] = array('text' => '⏭️ Next', 'callback_data' => 'uploads_page_' . ($current_page + 1));
        if (!empty($row)) $keyboard['inline_keyboard'][] = $row;
        $keyboard['inline_keyboard'][] = array(
            array('text' => '🎬 View Current Movie', 'callback_data' => 'view_current_movie'),
            array('text' => '🛑 Stop', 'callback_data' => 'uploads_stop')
        );
    }
...
if ($update) {
    ...
    if (isset($update['channel_post'])) {
        ...
        $text = isset($post['text']) ? $post['text'] : (isset($post['caption']) ? $post['caption'] : '');
        if (!empty(trim($text))) {
    ...
    if (isset($update['message'])) {
        ...
        if (!isset($users_data['users'][$user_id])) {
            $users_data['users'][$user_id] = array(
                'first_name' => isset($msg['from']['first_name']) ? $msg['from']['first_name'] : '',
                'last_name' => isset($msg['from']['last_name']) ? $msg['from']['last_name'] : '',
                'username' => isset($msg['from']['username']) ? $msg['from']['username'] : '',
                'joined' => date('Y-m-d H:i:s'),
                'last_active' => date('Y-m-d H:i:s')
            );
            $users_data['total_requests'] = (isset($users_data['total_requests']) ? $users_data['total_requests'] : 0) + 1;
        ...
        if (strpos($text, '/') === 0) {
            ...
            switch ($cmd) {
                ...
                case '/start':
                    ...
                    break;
                case '/stats':
                    if ($user_id == ADMIN_ID) admin_stats($chat_id);
                    break;
                case '/help':
...
if (php_sapi_name() === 'cli' || isset($_GET['setwebhook'])) {
    ...
    $res = apiRequest('setWebhook', array('url' => $webhook_url));
...
if (!$update) {
    ...
    echo "<p>Last Backup: " . (isset($stats['last_backup']) ? $stats['last_backup'] : 'Never') . " | Digest: " . (isset($stats['last_digest']) ? $stats['last_digest'] : 'Never') . "</p>";
